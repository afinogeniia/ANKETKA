...
# TODO: применть addRUle валидацию на стороне клиента        
			...
			$mform -> addElement('textarea', 'firstname', get_string("firstname"), 'wrap="virtual" rows = "1" cols = "30"');
			$mform->addRule('firstname', get_string('required'), 'required', null, 'client');
			$mform -> setDefault('firstname', $firstname);
			
			$mform -> addElement('textarea', 'middlename', 'Отчество', 'wrap="virtual" rows = "1" cols = "30"');
			$mform->addRule('middlename', get_string('required'), 'required', null, 'client');
			$mform -> setDefault('middlename', $middlename[1]);
			
			$mform -> addElement('textarea', 'lastname', get_string("lastname"), 'wrap="virtual" rows = "1" cols = "30"');
			$mform->addRule('lastname', get_string('required'), 'required', null, 'client');
			$mform -> setDefault('lastname', $lastname);
			
			//$mform -> addElement('textarea', 'institut', 'Институт', 'wrap="virtual" rows = "1" cols = "30"');
			$mform -> addElement('select', 'institut', 'Институт', array(
			...
			$mform -> addElement('select', 'kurs', 'Институт', array('1', '2', '3', '4', '5', '6'));
			$mform->addRule('kurs', get_string('required'), 'required', null, 'client');
			
			$mform -> addElement('textarea', 'group', get_string("group"), 'wrap="virtual" rows = "1" cols = "30"');
			$mform->addRule('group', get_string('required'), 'required', null, 'client');
			
			$mform -> addElement('textarea', 'phone', 'Телефон', 'wrap="virtual" rows = "1" cols = "30"');
			...
			$mform -> addElement('select', 'activity', 'Тип', array('учебная деятельность', 'научно-исследовательская деятельность', 'общественная деятельность', 'культурно-творческая деятельность', 'спортивная деятельность'));
			$mform->addRule('activity', get_string('required'), 'required', null, 'client');
			
			$mform -> addElement('selectyesno', 'received', 'Получали ли стипендию повышенную в предыдущем семестре');
